<?php
     ...
     class Usuario {
        ...
        private $senha;
        private $mensagem;
        //Define uma constante que indica o número de registros a serem exibidos por página, 
        ...
        public function inserirUsuario($dados){
         ...
         try{
            ...
            $cst =  $this->con->conectar()->prepare("INSERT INTO 
            usuario (nome,email,senha,mensagem) VALUES (:nome,:email,:senha,:mensagem) ");
        ...
        public function selecionaId($dado) {
                try{
                    ...
                    ("SELECT id, nome, email, senha, mensagem FROM usuario 
                    WHERE id = :idCliente ");
        ...
        public function editarUsuario($dados){
                try{
                        ...
                        $cst = $this->con->conectar()->prepare("UPDATE usuario 
                        SET nome = :nome , email = :email, senha = :senha, mensagem = :mensagem 
                        WHERE id = :idCliente");
                        $cst->bindParam(":idCliente" , $this->id , PDO::PARAM_INT);
                        $cst->bindParam(":nome" , $this->nome , PDO::PARAM_STR);
                        $cst->bindParam(":email" , $this->email , PDO::PARAM_STR);
        ...
        public function deletarId($dado){
                try{
                        ...
                        $cst = $this->con->conectar()->prepare("DELETE FROM usuario 
                        WHERE id= :idCliente");
                        $cst->bindParam(":idCliente" , $this->id, PDO::PARAM_INT);
                            if($cst->execute()){
                                return "ok";
        ...
        public function logarUsuarios($dados){
                ...
                try{ 
                        $cst = $this->con->conectar()->prepare("SELECT id, email, senha  
        ...
        {
                $cst = $this->con->conectar()->prepare("SELECT id,email,senha,nome FROM usuario WHERE id=:id");
